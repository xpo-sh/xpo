...
pub async fn login(provider: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(error_desc) = extract_query_param(path, "error_description") {
        let desc = percent_decode(&error_desc);
        let safe_desc: String = desc
            .chars()
            .filter(|c| c.is_alphanumeric() || *c == ' ')
            .collect();
        let redirect_url = format!(
            "https://xpo.sh/cli/error?msg={}",
            safe_desc.replace(' ', "+")
        );
        let response = format!(
            "HTTP/1.1 302 Found\r\nLocation: {}\r\nConnection: close\r\nContent-Length: 0\r\n\r\n",
        ...
        return Err(format!("login failed: {}", desc).into());
    }
    ...
    let redirect_url = "https://xpo.sh/cli/success";
    let response = format!(
        "HTTP/1.1 302 Found\r\nLocation: {}\r\nConnection: close\r\nContent-Length: 0\r\n\r\n",
        redirect_url
    );
